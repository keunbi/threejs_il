...
export default function example(){
    ...
    function draw(){    
        // console.log(clock.getElapsedTime()); // 시간의 경과. 초단위로 찍힘
        const time = clock.getElapsedTime();

        // mesh.rotation.y += 0.1; // 박스 돌림 (각도는 Radian을 사용 / 360도는 2파이)
        // mesh.rotation.y += THREE.MathUtils.degToRad(1); // degree값을 radian으로 변환해주는 메소드. 우리가 아는 각도 1도를 의미함
        mesh.rotation.y = time; // 속도 늘리고 싶으면 숫자 곱해줌 ( 2 * time ) / clock 이용해 애니메이션 주면 어느 기기든 동일하게 횟수 등 진행됨

...
export default function example(){
    ...
    camera.position.y = 1;
    camera.position.z = 6; // 카메라 위치 지정해주지 않으면 (0,0,0)에 위치함. 카메라가 내 쪽으로 5만큼 온 것. 물체에서 멀어진 것
    scene.add(camera);
    ...
    for (let i = 0; i < 10; i++){
        ...
        // Math.random() 은 0~1 사이 수이기 때문에 5를 곱해주면 다 오른쪽에 위치하게되므로 5의 절반을 빼줌
    ...
    function draw(){    
        ...
        meshes.forEach(item => { // 각 mesh의 item변수에 접근
